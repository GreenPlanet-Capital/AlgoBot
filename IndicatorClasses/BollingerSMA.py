...
class BollingerSMA:
    ...
    def __init__(self, dataframe_input, lookback_period, multiplier = 1.7, sensitivity = 1.1, absolute_sensitivity = 90):
...
#######################
    ...
    def indicator_generator(self):
        ...
        indic_columnhead = 'SMA ' + str(n) + ' ' + str(m)
        df_indicators[indic_columnhead] = temp_list
        
        sma = [None for i in range(n)]
        bol_up = [None for i in range(n)]
        bol_down = [None for i in range(n)]
        ...
        for i in range(len(df) - n):
            ...
            bol_up.append(sma_val + (m*sigma))
            bol_down.append(sma_val - (m*sigma))
            sma.append(sma_val)
...
#######################
    ...
    def signal_generation(self, indic_name = 'BOLEMA'):
        ...
        df_internal['UPBOUND'] = indic_df['BOLUPSMA ' + str(n) + ' ' + str(m)]
        df_internal['DOWNBOUND'] = indic_df['BOLDOWNSMA ' + str(n) + ' ' + str(m)]
        df_internal['MIDLINE'] = indic_df['SMA ' + str(n) + ' ' + str(m)]
        ...
        df_out['BOLSMA SIGNAL ' + str(n) + ' ' + str(m)] = scaled_interest_list_out
        
        #signum truth table construction
        indic_mean = df_out['BOLSMA SIGNAL ' + str(n) + ' ' + str(m)].mean()
        absolute_mean = 0
        indic_std = df_out['BOLSMA SIGNAL ' + str(n) + ' ' + str(m)].std()
        
        df_internal['BOLSMA SIGNUM BUY ' + str(n) + ' ' + str(m)] = df_out['BOLSMA SIGNAL ' + str(n) + ' ' + str(m)] >  (indic_mean + indic_std * sensitivity)
        df_internal['BOLSMA SIGNUM SELL ' + str(n) + ' ' + str(m)] = df_out['BOLSMA SIGNAL ' + str(n) + ' ' + str(m)] <=  (indic_mean - indic_std * sensitivity)
        df_internal['ABSOLUTE BOLSMA SIGNUM BUY ' + str(n) + ' ' + str(m)] = df_out['BOLSMA SIGNAL ' + str(n) + ' ' + str(m)] >  (absolute_mean + absolute_std * sensitivity)
        df_internal['ABSOLUTE BOLSMA SIGNUM SELL ' + str(n) + ' ' + str(m)] = df_out['BOLSMA SIGNAL ' + str(n)+ ' ' + str(m)] <=  (absolute_mean - absolute_std * sensitivity)
    
        #indicator signum
        long = list(df_internal['BOLSMA SIGNUM BUY ' + str(n) + ' ' + str(m)])
        short = list(df_internal['BOLSMA SIGNUM SELL ' + str(n) + ' ' + str(m)])
        ...
        df_out['BOLSMA SIGNUM ' + str(n) + ' ' + str(m)] = indic_out
        
        #absolute signum
        abs_long = list(df_internal['ABSOLUTE BOLSMA SIGNUM BUY ' + str(n) + ' ' + str(m)])
        abs_short = list(df_internal['ABSOLUTE BOLSMA SIGNUM SELL ' + str(n) + ' ' + str(m)])
        ...
        df_out['ABSOLUTE BOLSMA SIGNUM ' + str(n) + ' ' + str(m)] = abs_out
...
#######################
    ...
    def train_test(self, indic_name = 'BOLSMA', stop_percent = 0.05):
        df = self.dataframe_input
