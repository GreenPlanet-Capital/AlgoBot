...
Naming Convention of DataFrame Columns: 
    Indicator Generated DataFrame head: CCI + lookback_period + dev_weight
    Signal Generated DataFrame head: CCI SIGNAL + lookback_period + dev_weight
    Signum Generated DataFrame head: CCI SIGNUM + lookback_period + dev_weight
...
'''
'''
Inputs: dataframe_input, lookback_period, dev_weight = 0.15, sensitivity = 1, absolute_sensitivity = 50
...
class CommodityChannelIndex:
    ...
    def __init__(self, dataframe_input, lookback_period, dev_weight = 0.15 ,sensitivity = 1, absolute_sensitivity = 50):
...
#######################
    ...
    def indicator_generator(self):       
        ...
        indic_columnhead = 'CCI ' + str(n) + ' ' + str(m)
        df_indicators[indic_columnhead] = temp_list
            
        cci_list = [None for i in range(n)]
        
        initial_start_ctr = 0
        initial_end_ctr = n
        
        for i in range(len(df) - n):
            typ_price = df['TYP PRICE'].iloc[initial_end_ctr]
            price_list = list(df['TYP PRICE'].iloc[initial_start_ctr : initial_end_ctr])
            sum_var = sum(price_list)
            sma_val = sum_var/n
            std_dev = std(price_list)
            
            cci_val = (typ_price - sma_val)/(m * std_dev)
            cci_list.append(cci_val)
            
            initial_start_ctr += 1
            initial_end_ctr += 1

        df_indicators[indic_columnhead] = cci_list
...
#######################
    ...
    def signal_generation(self, indic_name = 'CCI'):
        ...
        indic_list = list(indic_df[indic_name + ' ' + str(n) + ' ' + str(m)])
        ...
        df_out = pd.DataFrame()
        df_out[indic_name + ' SIGNAL' + ' ' + str(n) + ' ' + str(m)] = scaled_signal_list
        
        #signum truth table construction
        indic_mean = df_out[indic_name + ' SIGNAL ' + str(n) + ' ' + str(m)].mean()
        absolute_mean = 0
        indic_std = df_out[indic_name +  ' SIGNAL ' + str(n) + ' ' + str(m)].std()
        absolute_std = self.absolute_sensitivity
        
        df_internal[indic_name + ' SIGNUM BUY ' + str(n) + ' ' + str(m)] = df_out[indic_name + ' SIGNAL ' + str(n) + ' ' + str(m)] >  (indic_mean + indic_std * sensitivity)
        df_internal[indic_name + ' SIGNUM SELL ' + str(n) + ' ' + str(m)] = df_out[indic_name + ' SIGNAL ' + str(n) + ' ' + str(m)] <=  (indic_mean - indic_std * sensitivity)
        df_internal['ABSOLUTE ' + indic_name + ' SIGNUM BUY ' + str(n) + ' ' + str(m)] = df_out[indic_name + ' SIGNAL ' + str(n) + ' ' + str(m)] >  (absolute_mean + (absolute_std * sensitivity))
        df_internal['ABSOLUTE ' + indic_name + ' SIGNUM SELL ' + str(n) + ' ' + str(m)] = df_out[indic_name + ' SIGNAL ' + str(n) + ' ' + str(m)] <=  (absolute_mean - (absolute_std * sensitivity))
        
        #indicator signum
        long = list(df_internal[indic_name + ' SIGNUM BUY ' + str(n) + ' ' + str(m)])
        short = list(df_internal[indic_name + ' SIGNUM SELL ' + str(n) + ' ' + str(m)])
        ...
        df_out[indic_name + ' SIGNUM ' + str(n) + ' ' + str(m)] = indic_out
        
        #absolute signum
        abs_long = list(df_internal['ABSOLUTE ' + indic_name + ' SIGNUM BUY ' + str(n) + ' ' + str(m)])
        abs_short = list(df_internal['ABSOLUTE ' + indic_name + ' SIGNUM SELL ' + str(n) + ' ' + str(m)])
        ...
        df_out['ABSOLUTE ' + indic_name + ' SIGNUM ' + str(n) + ' ' + str(m)] = abs_out
...
#######################
    ...
    def train_test(self, indic_name = 'CCI', stop_percent = 0.05):
        df = self.dataframe_input
