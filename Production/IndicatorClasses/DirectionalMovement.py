...
Naming Convention of DataFrame Columns: 
    Indicator Generated DataFrame head: OSC DIRMOV + lookback_period1 + DIRMOV + lookback_period2 
    Signal Generated DataFrame head: OSC DIRMOV + lookback_period1 + DIRMOV SIGNAL + lookback_period2
    Signum Generated DataFrame head: OSC DIRMOV + lookback_period1 + DIRMOV SIGNUM + lookback_period2
...
'''
'''
Inputs: dataframe_input, lookback_period1, lookback_period2, sensitivity = 1.2, absolute_sensitivity = 50 
...
#######################
    ...
    def indicator_generator(self):
        def dirmov(dataframe_input, lookback_period):
            df = dataframe_input
            n = lookback_period
            ...
            for i in range(len(df) - n):
                ...
                present_price = df['TYP PRICE'].iloc[initial_end_ctr]
                present_price = math.log(present_price,10) * 1000
                lookback_price = df['TYP PRICE'].iloc[initial_start_ctr]
                lookback_price = math.log(lookback_price,10) * 1000
                dirmov_val = (present_price - lookback_price) / n 
                dirmov.append(dirmov_val)
                    
                initial_start_ctr += 1
                initial_end_ctr += 1

            df_indicators[indic_columnhead] = dirmov
        ...
        shorterterm_df = dirmov(df, short_lookback)
...
#######################
    ...
    def train_test(self, stop_percent = 0.05):
        ...
        indic_name = 'OSC DIRMOV ' + str(self.lookback_period1) + ' DIRMOV'
