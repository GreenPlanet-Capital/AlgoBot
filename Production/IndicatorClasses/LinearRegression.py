...
Naming Convention of DataFrame Columns: 
    Indicator Generated DataFrame head: LINREG SLOPE + lookback_period 
    Signal Generated DataFrame head: LINREG SLOPE SIGNAL + lookback_period
    Signum Generated DataFrame head: LINREG SLOPE SIGNUM + lookback_period
...
Usage Notes:
* Slope may be used as a trend indicator
* Flat slope, combined with a breakout signal, may be used as a trend indicator.
...
#######################
    ...
    def indicator_generator(self):
        ...
        for i in range(len(df) - n):
            ...
            Sxy = 0
            Sxx = 0
            for (x,y) in zip(x_list,price_list):
                Sxy += (x - x_mean)*(y - y_mean)
                Sxx += (x - x_mean)*(x - x_mean)
            slope_val = Sxy/Sxx
            lin_reg.append(slope_val) 
            
            initial_start_ctr += 1
            initial_end_ctr += 1

        df_indicators[indic_columnhead] = lin_reg
...
#######################
    ...
    def signal_generation(self, indic_name = 'LINREG SLOPE'):
...
#######################
    ...
    def train_test(self, indic_name = 'LINREG SLOPE', stop_percent = 0.05):
        df = self.dataframe_input
