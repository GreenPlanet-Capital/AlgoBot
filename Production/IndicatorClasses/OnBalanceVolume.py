...
Naming Convention of DataFrame Columns: 
    Indicator Generated DataFrame head: MA OBV + lookback_period
    Signal Generated DataFrame head: MA OBV SIGNAL + lookback_period
    Signum Generated DataFrame head: MA OBV SIGNUM + lookback_period
...
#######################
    ...
    def indicator_generator(self):
        ...
        for i in range(len(df)):
            if (df_shifted['CLOSE'].iloc[i] > df['CLOSE'].iloc[i]):
                obv.append(-(df['VOLUME'].iloc[i]))
            elif (df_shifted['CLOSE'].iloc[i] == df['CLOSE'].iloc[i]):
                obv.append(0)
            else:
                obv.append(df['VOLUME'].iloc[i])
        
        df_indicators['RAWOBV'] = obv
        df_shifted1 = df_indicators.shift(1)
        df_indicators['OBV'] = df_shifted1['RAWOBV'] + df_indicators['RAWOBV']
        df_indicators['MA OBV ' + str(n)] = df_indicators['OBV'].rolling(n).mean()
...
#######################
    ...
    def signal_generation(self, indic_name = 'MA OBV'):
...
#######################
    ...
    def train_test(self, indic_name = 'MA OBV', stop_percent = 0.05):
        df = self.dataframe_input
