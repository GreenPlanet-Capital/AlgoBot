...
Naming Convention of DataFrame Columns: 
    Indicator Generated DataFrame head: SMA OSC + lookback_period
    Signal Generated DataFrame head: SMA OSC SIGNAL + lookback_period
    Signum Generated DataFrame head: SMA OSC SIGNUM + lookback_period
...
Usage Notes:
* STMA and LTMA time periods may be variable
* Moving Averages Taken may be EMA, SMA or WMA
* Can be used for a whole class of indicators
* Each Indicator can be used if it reaches a certain threshold value for correlation with the previous comparison, below whoch it would be considered a useful indicator
...
#######################
    ...
    def indicator_generator(self):
        ...
        def sma(dataframe_input, lookback_period):
            df = dataframe_input
            n = lookback_period
            ...
            for i in range(len(df) - n):
                ...
                sum_var = sum(list(df['TYP PRICE'].iloc[initial_start_ctr : initial_end_ctr]))
                sma_val = sum_var/n
                sma.append(sma_val)
...
#######################
    ...
    def signal_generation(self, indic_name = 'SMA OSC'):
...
#######################
    ...
    def train_test(self, indic_name = 'SMA OSC', stop_percent = 0.05):
        df = self.dataframe_input
