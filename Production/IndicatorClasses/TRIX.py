...
Naming Convention of DataFrame Columns: 
    Indicator Generated DataFrame head: TRIX + lookback_period 
    Signal Generated DataFrame head: TRIX SIGNAL+ lookback_period 
    Signum Generated DataFrame head: TRIX SIGNUM + lookback_period 
...
Usage Notes:
* When TRIX is used as a momentum indicator, a positive value suggests momentum is increasing while a negative value suggests momentum is decreasing.
...
* Divergence between price and TRIX can indicate significant turning points in the market.
...
#######################
    ...
    def indicator_generator(self):    
        ...
        df_out = ema(df,n)
        df_org = df_out
        df_shifted = df_out.shift(1)
        
        df_indicators = pd.DataFrame()
        indic_columnhead = 'TRIX ' + str(n)
        reference_column = 'EMA ' + str(n)
        df_indicators[indic_columnhead] = (df_org[reference_column] - df_shifted[reference_column])*1000/df_shifted[reference_column]
...
#######################
    ...
    def signal_generation(self, indic_name = 'TRIX'):
...
#######################
    ...
    def train_test(self, indic_name = 'TRIX', stop_percent = 0.05):
        df = self.dataframe_input
